...
graph LR;
Train Xtract Sampler Classification Models-->Train Regression Models to estimate size and time;
Train Regression Models to estimate size and time-->Calculate objective function for each file in directory;
Calculate objective function for each file in directory-->Insert file into a queue implemented as a heap sorted by value outputted from objective function;
Insert file into a queue implemented as a heap sorted by value outputted from objective function->Pop from queue highest values;
