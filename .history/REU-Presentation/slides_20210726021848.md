...
graph LR;
A[Train Xtract Sampler Classification Models]
B[Train Regression Models to estimate size and time]
C[<br>Calculate objective function for each file in directory]
D[Insert file into a queue implemented as a heap sorted by value outputted from objective function]
